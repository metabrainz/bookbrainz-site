...
describe('Merge routes', () => {
	describe('/add route', () => {
		...
		it('should throw an 400 error if adding an invalid BBID', (done) => {
			agent.get(`/merge/add/${inValidBBID}`).end((err, res) => {
		...
		it('should throw an 404 error if BBID is absent (invalid route)', (done) => {
			agent.get('/merge/add/').end((err, res) => {
				expect(err).to.be.null;
				expect(res).not.to.redirect;
		...
		it('should throw a 404 error if entity does not exist', (done) => {
			agent.get(`/merge/add/${nonExistingEntity}`).end((err, res) => {
				expect(err).to.be.null;
				expect(res).to.have.status(404);
		...
		it('should add an entity to the merge queue', (done) => {
			agent
				...
				.then((res) => {
					...
					agent.get('/merge/submit').end((err, response) => {
						expect(err).to.be.null;
						expect(response).to.have.status(409);
						expect(response.res.statusMessage).to.equal(
							'You must have at least 2 entities selected to merge'
						);
						done();
					});
				})
	...
	describe('/cancel route', () => {
		...
		it('should clear the merge queue', (done) => {
			agent
				.get(`/merge/add/${aBBID}`)
				.set('referer', `/author/${aBBID}`)
				.then(() => agent.get(`/merge/add/${bBBID}`).set('referer', `/author/${bBBID}`))
				.then(() => agent.get('/merge/submit'))
				.then((res) => {
					// expect mergequeue to exist and have two items
					expect(res).to.be.ok;
					return agent.get('/merge/cancel').set('referer', `/author/${bBBID}`);
				})
				.catch(done)
				.then(() => {
					agent
						...
						.end((error, response) => {
							...
							expect(response).to.have.status(409);
							expect(response.res.statusMessage).to.equal(
								'No entities selected for merge'
							);
							done();
						});
				});
