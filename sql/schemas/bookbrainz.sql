BEGIN;
	...
	(
		...
		WITHOUT TIME ZONE NOT NULL DEFAULT timezone
		('UTC'::TEXT, now
		()),
	active_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT timezone
		('UTC'::TEXT, now
		()),
	type_id INT NOT NULL,
	gender_id INT,
	area_id INT,
	revisions_applied INT NOT NULL DEFAULT 0 CHECK
		(revisions_applied >= 0),
	revisions_reverted INT NOT NULL DEFAULT 0 CHECK
		(revisions_reverted >= 0),
	total_revisions INT NOT NULL DEFAULT 0 CHECK
		(total_revisions >= 0),
	title_unlock_id INT
);
	...
	profile_rank SMALLINT
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.import
					(
    id SERIAL PRIMARY KEY,
    type bookbrainz.entity_type NOT NULL
);
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.creator_import_header
					(
	...
	data_id INT NOT NULL
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.edition_import_header
					(
	import_id INT PRIMARY KEY,
	data_id INT NOT NULL
);
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.publication_import_header
					(
	import_id INT PRIMARY KEY,
	data_id INT NOT NULL
);
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.publisher_import_header
					(
	import_id INT PRIMARY KEY,
	data_id INT NOT NULL
);
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.work_import_header
					(
	import_id INT PRIMARY KEY,
	data_id INT NOT NULL
);
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.discard_votes
					(
    import_id INT NOT NULL,
    editor_id INT NOT NULL,
    voted_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT timezone
					('UTC'::TEXT, now
					()),
    PRIMARY KEY
					(
        ...
        editor_id
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.origin_source
					(
    id SERIAL PRIMARY KEY,
    name TEXT NOT NULL CHECK
					...
					CREATE TABLE
					IF NOT EXISTS bookbrainz.link_import
					(
    import_id INT,
    origin_source_id INT NOT NULL,
    origin_id TEXT NOT NULL CHECK
					(origin_id <> ''),
    imported_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT timezone
					('UTC'::TEXT, now
					()),
    ...
    PRIMARY KEY
        ...
        origin_id
					...
					CREATE VIEW bookbrainz.creator
					AS
						SELECT
							e.bbid, cd.id AS data_id, cr.id AS revision_id, (cr.id = c
					.master_revision_id) AS master, cd.annotation_id, cd.disambiguation_id,
	...
	WHERE e.type = 'Creator';
					...
					CREATE VIEW bookbrainz.edition
					AS
						SELECT
							e.bbid, edd.id AS data_id, edr.id AS revision_id, (edr.id = ed
					.master_revision_id) AS master, edd.annotation_id, edd.disambiguation_id,
	...
	WHERE e.type = 'Edition';
					...
					CREATE VIEW bookbrainz.work
					AS
						SELECT
							e.bbid, wd.id AS data_id, wr.id AS revision_id, (wr.id = w
					.master_revision_id) AS master, wd.annotation_id, wd.disambiguation_id,
	...
	WHERE e.type = 'Work';
					...
					CREATE VIEW bookbrainz.publisher
					AS
						SELECT
							e.bbid, psd.id AS data_id, psr.id AS revision_id, (psr.id = ps
					.master_revision_id) AS master, psd.annotation_id, psd.disambiguation_id,
	...
	WHERE e.type = 'Publisher';
					...
					CREATE VIEW bookbrainz.publication
					AS
						SELECT
							e.bbid, pcd.id AS data_id, pcr.id AS revision_id, (pcr.id = pc
					.master_revision_id) AS master, pcd.annotation_id, pcd.disambiguation_id,
	...
	WHERE e.type = 'Publication';
					...
					CREATE VIEW bookbrainz.creator_import
					AS
						SELECT
					...
					CREATE VIEW bookbrainz.edition_import
					AS
						SELECT
							import.id AS import_id,
					...
					CREATE VIEW bookbrainz.publisher_import
					AS
						SELECT
							import.id AS import_id,
					...
					CREATE VIEW bookbrainz.publication_import
					AS
						SELECT
							import.id AS import_id,
					...
					CREATE VIEW bookbrainz.work_import
					AS
						SELECT
							import.id as import_id,
