	...
	try {
		...
		if (entity.dataId) {
			...
			if (entity.seriesOrderingType.label === 'Manual') {
				...
				relationships.sort((a, b) => (a.number > b.number ? 1 : b.number > a.number ? -1 : 0));
			}
			const entities = relationships.map((rel) => (
				{...rel.source, displayNumber: true,
					number: typeof rel.number === 'number' ? null : rel.number,
					position: typeof rel.position === 'number' ? null : rel.position}
			));
