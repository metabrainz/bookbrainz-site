...
function getEntityRelations(entityType) {
	const authorRelations = ['defaultAlias.language', 'disambiguation', 'authorType', 'gender'];
	...
	const publisherRelations = ['defaultAlias.language', 'disambiguation', 'publisherType', 'area'];
...
router.post(
	...
	middleware.validateCollectionParams,
	collectionCreateOrEditHandler
);

/* If the route specifies a CollectionId, load the Collection for it. */
router.param('collectionId', middleware.makeCollectionLoader());
...
router.get('/:collectionId', auth.isAuthenticatedForCollectionView, async (req, res, next) => {
	try {
		...
		const isCollaborator =
			req.user &&
			collection.collaborators.filter((collaborator) => collaborator.id === req.user.id)
				.length;
	...
	async (req, res, next) => {
		try {
			...
			const entitiesPromise = items.map(async (item) => ({
				addedAt: item.added_at,
				...(await orm.func.entity.getEntity(
					orm,
...
router.post(
	...
	async (req, res, next) => {
		try {
			...
			if (!isAllowedToEdit) {
				if (
					collaboratorIdsToRemove.length === 1 &&
					parseInt(collaboratorIdsToRemove[0], 10) === userId
				) {
