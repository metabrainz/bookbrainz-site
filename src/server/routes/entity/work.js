...
const createOrEditHandler = makeEntityCreateOrEditHandler('work', transformNewForm, 'typeId');

const mergeHandler = makeEntityCreateOrEditHandler('work', transformNewForm, 'typeId', true);
...
router.get(
	...
	(req, res, next) => {
		...
		makePromiseFromObject(propsPromise).then(render).catch(next);
	}
);
...
router.param(
	'bbid',
	middleware.makeEntityLoader('Work', ['workType', 'languageSet.languages'], 'Work not found')
);
